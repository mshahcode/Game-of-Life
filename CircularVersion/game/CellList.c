...
int CL_neighbours(int row,int col,Cell* neighbours,CellList* cl){
    ...
    if(CL_belong(row-1,col,cl)){
        ...
        neighbours[size++] = cl->board1[cl->row-1][col]; /// circular
    }

    if(CL_belong(row+1,col,cl)){
        neighbours[size++] = cl->board1[row+1][col];
    }else{
        neighbours[size++] = cl->board1[0][col]; /// circular
    }
    ...
    if(CL_belong(row,col+1,cl)){
        ...
        neighbours[size++] = cl->board1[row][0]; /// circular
    }

    if(CL_belong(row,col-1,cl)){
        neighbours[size++] = cl->board1[row][col-1];
    }else{
        neighbours[size++] = cl->board1[row][cl->col-1]; // circular
    }
